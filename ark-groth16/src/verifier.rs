...
impl<E: Pairing, QAP: R1CSToQAP> Groth16<E, QAP> {
    ...
    pub fn prepare_inputs(
        ...
        if (public_inputs.len() + 1) != pvk.vk.t_v_g1.len() {
            return Err(SynthesisError::MalformedVerifyingKey);
        }

        let mut g_ic = pvk.vk.t_v_g1[0].into_group();
        for (i, b) in public_inputs.iter().zip(pvk.vk.t_v_g1.iter().skip(1)) {
    ...
    pub fn verify_proof(
        ...
        let prepare_input_time = ark_std::time::Instant::now();
        let prepared_inputs = Self::prepare_inputs(pvk, public_inputs)?;
        let prepare_input_time = prepare_input_time.elapsed();
        println!("Prepare input time: {:?}", prepare_input_time);

        let verify_time = ark_std::time::Instant::now();
        let result = Self::verify_proof_with_prepared_inputs(pvk, proof, &prepared_inputs)?;
        let verify_time = verify_time.elapsed();
        println!("Verify time with prepared inputs: {:?}", verify_time);
