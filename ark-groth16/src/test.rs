use crate::{
    fft_handler::fft_bench_test,
    prepare_verifying_key, Groth16,
};
...
pub fn test_fft_bench<E: Pairing>(domain_size: usize) {
    let mut rng = ark_std::rand::rngs::StdRng::seed_from_u64(test_rng().next_u64());
    // let fft_info: CustomParaFFT<E> = CustomParaFFT::<E>::new(domain_size,
    // domain.group_gen());
    println!("------------------------------------------------------------------------");
    fft_bench_test::<E, _>(domain_size, &mut rng);
    println!("------------------------------------------------------------------------\n");
}
...
mod fft_bench {
    use super::test_fft_bench;
    use ark_bn254::Bn254;
    // use
    #[test]
    fn go_test() {
        println!("HERE_DIY");
        test_fft_bench::<Bn254>(1<<10);
        // test_fft_bench::<Bn254>(1<<11);
        test_fft_bench::<Bn254>(1 << 12);
        // test_fft_bench::<Bn254>(1<<13);
        test_fft_bench::<Bn254>(1<<14);
        // test_fft_bench::<Bn254>(1<<15);
        test_fft_bench::<Bn254>(1<<16);
        // test_fft_bench::<Bn254>(1<<17);
        // test_fft_bench::<Bn254>(1<<18);
        // test_fft_bench::<Bn254>(1<<20);
        // ::<Bn254>(1<<20);
        println!("HERE_DIY");
    }
}
